...
Searching for an enemy and a 1v1 fight with HP (GDD 7.1, 7.2). Pure TypeScript,
no Phaser. The seeded Rng lives in the state, so the same seed always gives the
same fights.

M2 scope: hit/miss and damage via `core/combat`. When the enemy dies the next
search starts automatically. When the squirrel is defeated she is back to full
HP in `idle` (placeholder until real death in M3). No regeneration yet (M3).

## Public API
- `createEncounterConfig(input)` – validated config from design values (seconds,
  1-decimal stats, whole percentages); see `core/content/encounterInput.ts`.
- `createEncounter(config, rng)` – new state in phase `idle`, squirrel at full HP.
...
- `makePeace(state)` – player pressed **Peace!**: from `searching` or `fighting` straight back to
  `idle` (event `peaceMade { from }`; the enemy leaves, no XP/loot, HP is kept). Does nothing while `idle`.
...
- `hpFraction(state, config, who)` – 0..1 for HP bars.
...
## Depends on
- `core/time` (`TICK_MS`, `secondsToMs`), `core/combat`, `core/rng` (types).
